...
function Service() {
  return (
    <div className="py-4 px-10 xl2:px-8 xl3:px-12 space-y-8 xl2:space-y-12">
      <div>
        <h1 className="text-2xl ml:text-3xl lg:text-4xl 2xl:text-5xl xl2:text-6xl xl3:text-8xl font-semibold text-center  mb-2 text-theme">
          My Services
        </h1>
        <p className="text-center w-2/3 xl:w-1/2 xl3:w-2/3 mx-auto mb-8 text-secondary text-xs ml:text-sm lg:text-lg 2xl:text-xl xl2:text-2xl xl3:text-3xl">
      ...
      <div className="flex justify-center gap-4 ml:gap-8 xl:gap-12 3xl:gap-16 xl2:gap-11 xl3:gap-20 flex-wrap content-between">
        {coreSkills.map((language, index) => (
          <div
            key={index}
            className="w-[150px] ml:w-[175px] lg:w-[200px] lg2:w-[225px] xl:w-[250px] 2xl:w-[290px] 3xl:w-[300px] xl2:w-[350px] xl3:w-[400px] flex-wrap aspect-square glass p-4 flex flex-col justify-evenly"
          >
            <a href={language.url} target="_blank" rel="noopener noreferrer" className="flex flex-col justify-evenly h-full">
              ...
              <div className="text-sm lg:text-lg lg2:text-xl 2xl:text-2xl xl2:text-3xl xl3:text-4xl text-center font-medium">
                {language.name}
              </div>
              <p className="text-[8px] ml:text-[10px] lg:text-xs lg2:text-sm 2xl:text-base xl2:text-lg xl3:text-xl text-center text-wrap">
      ...
      <div className="flex justify-center flex-wrap content-between gap-6 xl:gap-8 mx-8">
        {additionalSkills.map((language, index) => (
          <div
            key={index}
            className="w-[75px] ml:w-[100px] xl:w-[125px] 2xl:w-[150px] xl2:w-[175px] xl3:w-[200px] aspect-square glass flex flex-col justify-evenly"
          >
            {" "}
            <a href={language.url} target="_blank" rel="noopener noreferrer" className="space-y-2">
              ...
              <div className="text-[8px] ml:text-xs lg:text-sm 2xl:text-base xl2:text-lg xl3:text-2xl text-center font-medium">{language.name}</div>
            </a>
          </div>
        ))}
      </div>
